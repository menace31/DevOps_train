...
def post_message():
    ...
    response = requests.post(f"{SAVE_URL}/save", json={"content": user_data['contenu']},timeout=5)
...
def execute_SQL():
    user_data = request.get_json()
    response = requests.post(f"{SAVE_URL}/exec", json={"content":user_data['query']},timeout=5)
    return jsonify({"gateway_status": "Transmis", "storage_response": response.text})
